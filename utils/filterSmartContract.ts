...
export const filterContract = (content: string) => {
  const regex =
  ...
  const events = parsed.filter((a) => {
    if (a?.match(/(\b(?=\w)function \w+\(?.+\))/)) {
      // return a.replace(/function|/);
    }
  });

  return { functions, events: [] } as IContractData;
};
...
export const getInheritedFunctions = (
